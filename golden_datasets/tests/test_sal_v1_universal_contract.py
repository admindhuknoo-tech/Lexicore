...
def test_portable_builtin_schema_validator_does_not_airgap_valid_payload(monkeypatch):
    """Portable/offline installs without jsonschema must still validate the frozen SAL contract."""
    import services.semantic_admission as salmod
    monkeypatch.setattr(salmod, 'Draft202012Validator', None)
    p = salmod.build_statement_contract('Apakah benar tanda tangan Saudara?', index=99, prefix='BAP')
    assert p['semantic_envelope']['semantic_type'] == 'QUESTION'
    assert p['semantic_envelope']['provenance'] != 'SYSTEM_CRITICAL_FALLBACK'
    assert p['admission_contract']['admissibility_state'] == 'REJECTED'
    salmod.validate_payload(p)


def test_short_bukti_reference_is_lead_only_not_alleged_act_or_evidence():
    p=build_statement_contract('bukti kerugian', index=120, prefix='LELY')
    assert p['semantic_envelope']['semantic_type']=='DOCUMENT_REFERENCE'
    assert p['admission_contract']['admissibility_state']=='LEAD_ONLY'
    assert 'Evidence Map' in p['admission_contract']['prohibited_routes']
    assert 'Alleged Act' in p['admission_contract']['prohibited_routes']


def test_mining_law_rejected_from_contract_and_civil_procedure_case():
    ok,reason=law_subject_matter_admissible(
        'Tata Cara Dan Syarat-Syarat Pengajuan Izin Usaha Pertambangan (IUP)',
        case_domains=['civil_contract','civil_procedure'],
        issue_text='kompetensi relatif gugatan wanprestasi',
        case_text='utang pinjaman somasi gugatan pengadilan negeri'
    )
    assert not ok
    assert reason=='SUBJECT_MATTER_MISMATCH_PERTAMBANGAN'


def test_cross_layer_enforcer_blocks_lead_only_from_evidence_and_alleged_act():
    from services.contract_enforcer import LexiCoreContractEnforcer
    p=build_statement_contract('surat perjanjian hutang piutang', index=121, prefix='LELY')
    p=LexiCoreContractEnforcer.enforce_routing(p)
    assert not LexiCoreContractEnforcer.route_allowed(p,'Evidence Map')
    assert not LexiCoreContractEnforcer.route_allowed(p,'Alleged Act')
    assert LexiCoreContractEnforcer.route_allowed(p,'Document Audit')
