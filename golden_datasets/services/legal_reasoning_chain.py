...
def _source_route_allows(result: Dict[str, Any], evidence_item: Dict[str, Any], route: str) -> bool:
    ...
    # Legacy rows have no SAL envelope; keep backward compatibility but do not
    # infer permission from text alone.
    return True
...
def build_reasoning_chain(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for idx, element in enumerate(elements, 1):
        ...
        evidence = _evidence_items(result, element, issue)
        counter = _counter_evidence(element, result)
        alleged_act = _clean(element.get("alleged_act") or element.get("prosecution_support") or "")
        ...
        row = {
            ...
            "legal_elements": {"element": element_name, "source_status": test_status, "owner_domain": _clean(element.get("owner_domain")), "owner_issue_id": _clean(element.get("owner_issue_id")), "status": "MAPPED"},
        ...
        for key, present in {
            ...
            "legal_elements": bool(element_name),
