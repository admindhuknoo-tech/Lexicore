...
def classify_semantic_type(text: str, source_item: Dict[str, Any] | None = None, *, posture: str = "", corpus: str = "") -> str:
    ...
    # Backlog A.2 — generic civil-pleading quality guard. It returns only an
    # existing frozen SAL semantic type and has no authority over routing/evidence.
    try:
        from extractors.pleading_cleaner import semantic_override as pleading_semantic_override
        pleading_hint = pleading_semantic_override(s, posture=posture, corpus=corpus, source_item=item)
        if pleading_hint.get("override_type") in SEMANTIC_TYPES:
            return str(pleading_hint["override_type"])
    except Exception:
        pass

    # Backlog A — BAP semantic-noise cleaner. This is a bounded typing hint only;
    # it cannot alter source text, routing, evidence admission, or identity.
    try:
        from extractors.bap_noise_cleaner import semantic_override
        bap_hint = semantic_override(s, posture=posture, corpus=corpus, source_item=item)
        if bap_hint.get("override_type") in SEMANTIC_TYPES:
            return str(bap_hint["override_type"])
    except Exception:
        # Fail safely to the frozen generic SAL classifier. The cleaner is a
        # quality layer, never a required path for contract validity.
        pass
    ...
    # Bare remedy/prayer/object phrases are not historical acts.  Preserve them
    # as unverified party propositions so Positive-Allow consumers can route
    # them to audit/action, never to Alleged Act or Evidence.
    if re.match(r"^\s*(?:ganti\s+rugi|pengembalian\s+(?:pinjaman|uang|barang)|pemulihan\s+hak|petitum|tuntutan)\b", s, re.I):
...
def build_statement_contract(text: str, *, index: int = 0, prefix: str = "GEN", source_item: Dict[str, Any] | None = None,
    ...
    s = _clean(text)
    st = classify_semantic_type(s, source_item, posture=posture)
...
def enrich_source_ledger_with_sal(ledger: Iterable[Dict[str, Any]], *, domain_contract: Dict[str, Any] | None = None, posture: str = "", raw_document_header: str = "", raw_extracted_text: str = "") -> List[Dict[str, Any]]:
    out = []
    document_corpus = raw_extracted_text or raw_document_header
    try:
        from extractors.bap_noise_cleaner import is_bap_context
        _bap_context = is_bap_context(posture=posture, corpus=document_corpus)
    except Exception:
        _bap_context = False
    try:
        from extractors.pleading_cleaner import is_civil_pleading_context
        _civil_pleading_context = is_civil_pleading_context(posture=posture, corpus=document_corpus)
    except Exception:
        _civil_pleading_context = False
    for idx, item in enumerate(ledger or []):
        ...
        if not text:
            continue
        _typing_source = dict(row)
        if _bap_context:
            _typing_source["_semantic_context"] = "BAP_INTERROGATION"
        elif _civil_pleading_context:
            _typing_source["_semantic_context"] = "CIVIL_PLEADING"
        # Pass the verified BAP context through the source item while keeping the
        # immutable ledger row untouched.
        sal = build_statement_contract(text, index=idx, prefix="LEDGER", source_item=_typing_source, domain_contract=domain_contract, posture=posture)
        from services.contract_enforcer import LexiCoreContractEnforcer
        sal = LexiCoreContractEnforcer.enforce_adversarial_context(sal, document_corpus)
        # Keep the frozen SAL schema authoritative after viewpoint migration.
        sal = validate_and_route_sal_payload(sal, text)
        ...
        row["sal_speaker_role"] = adv["speaker_role"]
        row["sal_position"] = adv["position"]
        row["sal_stance"] = adv["stance"]
        row["sal_epistemic_status"] = adv.get("epistemic_status", "UNSPECIFIED")
        out.append(row)
    return out
